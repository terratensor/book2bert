...
**Проблема:** Исходные файлы могут быть в разных кодировках. В нашем первом эксперименте (50 книг) мы столкнулись с Windows-1251. В militera корпусе все файлы в UTF-8, но мы оставили функцию конвертации как защиту — на случай, если в будущем появятся данные в других кодировках.
...
func ToUTF8(text []byte) (string, error) {
    ...
    // Пробуем декодировать из Windows-1251 (защита от старых форматов)
...
**Почему это важно:** 
- В основном корпусе (militera) кодировка UTF-8, конвертация не требуется
- Но функция осталась для совместимости с другими источниками данных
- Без правильной конвертации мы получили бы кракозябры вместо текста

```json
// Пример из первого эксперимента (Windows-1251, неверно прочитано):
{"text": "��������: ����������������� � �����"}

// После конвертации в UTF-8:
