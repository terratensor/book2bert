#!/usr/bin/env python3
"""
Обучение BERT-base с нуля с поддержкой:
...
def mask_tokens(input_ids, tokenizer, mlm_probability=0.15):
    ...
    # Создаем маску специальных токенов на том же устройстве
...
def train_epoch(model, dataloader, optimizer, scheduler, scaler, device, tokenizer, 
    ...
    """Обучение одной эпохи с поддержкой fp16 и gradient accumulation"""
    model.train()
    total_loss = 0
    accumulated_loss = 0
    ...
    for step, batch in enumerate(progress_bar):
        ...
        with torch.amp.autocast('cuda'):
            ...
            loss = outputs['loss']
            loss = loss / gradient_accumulation_steps  # нормализуем для аккумуляции
...
def main():
    parser = argparse.ArgumentParser(description="Train BERT-base with mixed precision")
    ...
    # Создаем выходную директорию с меткой времени
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = Path(args.output_dir) / f"base_bert_{timestamp}"
    ...
    if args.use_wandb:
        import wandb
        wandb.init(project="book2bert", name=f"base_bert_{timestamp}", config=config)
