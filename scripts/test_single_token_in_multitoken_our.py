...
class OurMultitokenMLMTester:
    def __init__(self, model_path, tokenizer_path, device="cuda"):
        ...
        self.model = BERTForMLM(bert, vocab_size=50000)
    ...
    def test_masked_token(self, text, word, token_index, top_k=10):
        ...
        masked_tokens[pos_idx] = "[MASK]"
        masked_text = ' '.join(masked_tokens).replace(' ##', '')
        
        # Токенизируем маскированный текст
        ...
        if len(mask_positions) == 0:
            return {
                'success': False,
                'error': 'Mask token not found after encoding',
                'word': word
            }
        ...
        predictions = [self.tokenizer.id_to_token(idx.item()) for idx in top_k_indices]
        predicted_token = predictions[0]
        
        # Очищаем от ## для сравнения
        clean_original = original_token.replace('##', '')
        clean_predicted = predicted_token.replace('##', '')
        is_correct = clean_original == clean_predicted
        
        return {
            'word': word,
            'word_tokens': [full_tokens[p] for p in positions],
            'masked_token': original_token,
            'masked_token_clean': clean_original,
            'mask_position': pos_name,
            'predicted_token': predicted_token,
            'predicted_token_clean': clean_predicted,
...
def main():
    ...
    tester = OurMultitokenMLMTester(args.model_path, args.tokenizer_path, device)
    
    print("\n" + "="*80)
    print(f"MLM ON MULTITOKEN WORDS (OUR MODEL)")
    print("="*80)
